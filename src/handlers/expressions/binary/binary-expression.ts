...
export function BinaryExpression(context: ISerializationContext, ast: import("estree").BinaryExpression, opts: ISerializationOptions): ISerializationResult {
    ...
    let code: string;
    if (ast.left.type == "BinaryExpression" || ast.left.type == "LogicalExpression" || ast.left.type == "SequenceExpression" || ast.left.type == "AssignmentExpression" || ast.left.type == "AwaitExpression") {
        code = "(" + left + ")";
    } else {
        code = left;
    }
    ...
    if (ast.right.type == "BinaryExpression" || ast.right.type == "LogicalExpression" || ast.right.type == "SequenceExpression" || ast.right.type == "AssignmentExpression" || ast.right.type == "AwaitExpression") {
        code += "(" + right + ")";
    } else {
        code += right;
    }
