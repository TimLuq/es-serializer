...
export function ConditionalExpression(context: ISerializationContext, ast: import("estree").ConditionalExpression, opts: ISerializationOptions): ISerializationResult {
    ...
    if (ast.test.type == "BinaryExpression" || ast.test.type == "LogicalExpression" || ast.test.type == "SequenceExpression" || ast.test.type == "AssignmentExpression" || ast.test.type == "AwaitExpression" ||  ast.test.type == "YieldExpression") {
        code = "(" + code + ")";
    }
    const d = opts.indent;
    if (d !== undefined) {
