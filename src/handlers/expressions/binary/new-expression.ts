...
export function NewExpression(context: ISerializationContext, ast: import("estree").NewExpression, opts: ISerializationOptions): ISerializationResult {
    ...
    const d = opts.indent;
    if (ast.callee.type == "BinaryExpression" || ast.callee.type == "LogicalExpression" || ast.callee.type == "SequenceExpression" || ast.callee.type == "AssignmentExpression" || ast.callee.type == "AwaitExpression" ||  ast.callee.type == "YieldExpression" || ast.callee.type == "ConditionalExpression") {
        if (d !== undefined) {
            code += " ";
        }
        code += "(" + callee + ")";
